---
title: "アセンブリおよび Itinerary 入り口サンプルがインストールされているアイテム |Microsoft ドキュメント"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: f90ce354-4be9-439d-ba0b-d22706be359b
caps.latest.revision: "2"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 34b9c6337a502a394dc01c54f3beda6cc8c9413c
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 09/20/2017
---
# <a name="assemblies-and-artifacts-installed-by-the-itinerary-on-ramp-sample"></a>アセンブリおよび Itinerary 入り口サンプルがインストールされているアイテム
次の表は、アセンブリおよびによってインストールされているアイテム、[!INCLUDE[esbToolkit](../includes/esbtoolkit-md.md)]行程入り口サンプルです。  
  
|場所|カテゴリ|コンポーネントの名前とバージョン|  
|--------------|--------------|---------------------------------------|  
|BizTalk アプリケーション GlobalBank.ESB|オーケストレーション|GlobalBank.ESB.Itinerary.Processes.ProcessAndRespond|  
|BizTalk アプリケーション GlobalBank.ESB|送信ポート|(なし)|  
|BizTalk アプリケーション GlobalBank.ESB|受信ポート|(なし)|  
|BizTalk アプリケーション GlobalBank.ESB|受信場所|(なし)|  
|BizTalk アプリケーション GlobalBank.ESB|スキーマ|(なし)|  
|BizTalk アプリケーション GlobalBank.ESB|パイプライン|(なし)|  
|BizTalk アプリケーション GlobalBank.ESB|リソース|(なし)|  
|BizTalk アプリケーション GlobalBank.ESB|ポリシー|(なし)|  
|BizTalk アプリケーション GlobalBank.ESB|マップ|(なし)|  
|BizTalk アプリケーション GlobalBank.ESB|アセンブリ|(なし)|  
|BizTalk アプリケーション GlobalBank.ESB|パイプライン コンポーネント|(なし)|