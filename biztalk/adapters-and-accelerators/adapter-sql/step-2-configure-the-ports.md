 ...
 **目標:** 物理ポートを作成するこの手順で[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]管理コンソール。 オーケストレーションで作成した各論理ポートに対する物理ポートを作成します。 次のポートを作成します。  
...
5. 変更の通知を受信するアダプターを構成するのには、次のバインド プロパティを設定することを確認、**従業員**テーブル。  
   ...
   |**NotificationStatement**|これを設定します。<br /><br /> `SELECT Employee_ID, Name FROM dbo.Employee WHERE Status=0`<br /><br /> **注:** 具体的には、次の Select ステートメントで示すように、列名をステートメントで指定する必要があります。 また、する必要があります常にテーブル名を指定、スキーマ名と共になど`dbo.Employee`します。|  
...
## <a name="see-also"></a>参照  
 [ステップ 1: オーケストレーションを展開します。](../../adapters-and-accelerators/adapter-sql/step-1-deploy-the-orchestration.md)   
