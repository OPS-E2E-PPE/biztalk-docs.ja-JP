...
**考えられる原因**: MLLP の 1 つを使用することができますのみハンドラーは単一のサーバー上の受信 (または送信) します。 さらに、URI が指定されている受信場所 (または送信ポート) (MLLP トランスポートのプロパティ内のホスト名) は、"localhost"をする必要がありますか、または受信 (または送信) のアダプター ハンドラーのホストのインスタンスでサーバー名が実行されています。  
  
**解像度**: 1 台のサーバーで同じ受信 (または送信) ハンドラーすべて MLLP の受信場所 (または送信ポート) を指定します。  
...
**考えられる原因**: The MSH と確認のスキーマ (MSH_25_GLO_DEF.xsd と ACK_24_GLO_DEF.xsd) は、複数のプロジェクトで配置されています。  
  
**解像度**: MSH_25_GLO_DEF.xsd と ACK_24_GLO_DEF.xsd が 1 つしかプロジェクトに追加されていることを確認します。  
...
**解像度**: 再起動[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]します。  
...
**解像度**: 確認 MSH マップ値[!INCLUDE[btaBTAHL71.3abbrevnonumber](../../includes/btabtahl71-3abbrevnonumber-md.md)]構成エクスプ ローラー。  
...
**解像度**: メッセージは、正しく書式設定された、指定済みの MSH11 フィールドであることを確認します。  
