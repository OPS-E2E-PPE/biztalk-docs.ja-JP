...
次の用語と定義がで使用される[!INCLUDE[adapteroracle](../../includes/adapteroracle-md.md)]です。    
...
**BFILE**  
Oracle データベースに外部のファイル システムに格納されている Lob のバイナリ ファイルにアクセスできるようにする Oracle データ型。 BFILE 列は、サーバーのファイル システム上のデータを含むファイル名とディレクトリの名前を表す BFILE ロケーターを格納します。

**バイナリ ラージ オブジェクト**  
1.  大規模なフィールドの値、予測不可能なテーブルのサイズおよびアプリケーションの観点から formless はデータによって特徴付けられた、ビットマップなど、データの大きな部分。
2.  データのブロックに関する情報を含む BLOB 構造を指定するキーワードです。
...
**ローカルの名前付けメソッド**  
Oracle データベース アダプターでサポートされているメソッドの名前付け Oracle です。 この名前付けの方法では、Oracle クライアントは、ローカル tnsnames.ora ファイル内のエントリに net サービス名を解決します。  
...
**名前付けの方法**  
Oracle の名前付け方法は、Oracle クライアントが Oracle データベース サービス (インスタンス) の接続情報を取得する方法を決定します。 Oracle Net Configuration Assistant を使用して特定の名前付け方法を使用する Oracle クライアントを構成することができます。 Oracle データベース アダプターには、ローカルの名前付け方法がサポートしています。

**net サービス名**  
Oracle データベースの接続情報を取得する Oracle クライアントで使用される別名です。 接続 URI 内の接続プロパティの 1 つとしては、net サービス名を指定します。  
...
**REF CURSOR**  
Oracle PL/SQL データ型の Oracle データベースに結果セットへのポインターを表します。 REF カーソルの種類は、入力と出力が、データのストリーミングを有効にしは大量の PL/SQL コード ブロックとの間のデータを転送するのに最適です。
