...
### <a name="solution"></a>解決方法  
 エラー メッセージに記載されている詳細情報も確認してください。 再起動[!INCLUDE[btsCoName](../../includes/btsconame-md.md)] [!INCLUDE[btsSQLServerNoVersion](../../includes/btssqlservernoversion-md.md)]™ この問題を解決する可能性があります。  
...
### <a name="possible-cause"></a>考えられる原因  
 [!INCLUDE[btaBTARN3.3abbrevnonumber](../../includes/btabtarn3-3abbrevnonumber-md.md)]® 管理コンソール (MMC) からアグリーメントとパートナーを削除する前に BtarnClean [!INCLUDE[btsCoName](../../includes/btsconame-md.md)]Clean ユーティリティを実行した場合、アイテムがまだ使用中であるため、一部のアイテムは登録を解除できません。  
...
1.  をクリックして**開始**、 をポイント**すべてのプログラム**、 をポイント**Microsoft SQL Server**、順にクリック**Enterprise Manager**です。  
  
2.  Enterprise Manager で展開**Microsoft SQL Servers**、 **SQL Server グループ**、 **(ローカル) (Windows NT)**、および**データ変換サービス**.  
  
3.  をクリックして**ローカル パッケージ**を右クリックして**BAM_DM_Message**、順にクリック**削除**です。  
  
4.  右クリック**BAM_DM_Process**、クリックして**削除**です。  
  
5.  をクリックして**開始**、 をクリックして**実行**、型**cmd**、順にクリック**ok**です。  
  
6.  コマンド プロンプトで、追跡ファイルを展開する次のコードを入力し、クリックして**OK**です。  
...
2.  行をコメント"127.0.0.1 localhost"の行の先頭に「#」を配置することによってです。 変更されたホスト ファイルを保存します。  
  
3.  をクリックして**再試行**エラー ダイアログ ボックス。  
...
### <a name="symptom"></a>現象  
 後に[!INCLUDE[BTARN_CurrentVersion_abbrev](../../includes/btarn-currentversion-abbrev-md.md)]にアクセスしようとすると、構成が完了した[http://localhost](http://localhost/)または[http://localhost/BtarnApp/RnifSend.aspx](http://localhost/BtarnApp/RnifSend.aspx)、次のようなエラーを受信する可能性があります。  
...
4.  選択**RpcProxy フィルター**、 をクリック**削除**です。  
  
5.  **[OK]**をクリックします。  
  
6.  をクリックして**開始**、 をクリックして**実行**、型**cmd**、順にクリック**ok**です。  
...
> [!NOTE]
>  以上の手順を実行した後で http://localhost または http://localhost/BtarnApp/RnifSend.aspx に再度アクセスすると、IIS が正常に機能していることを示す HTTP 400 メッセージが表示されます。  
...
### <a name="solution"></a>解決方法  
 HubScenario および HubHelper プロジェクトのアセンブリ キー ファイルを設定します。 詳細については、次を参照してください。 [HubScenario サンプル](../../adapters-and-accelerators/accelerator-rosettanet/hubscenario-sample.md)です。  
