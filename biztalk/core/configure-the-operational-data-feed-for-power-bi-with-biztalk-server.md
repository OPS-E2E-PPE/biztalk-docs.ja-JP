...
3. 次のテキストで置換`Default Web Site`、 `operationalDataServiceAppPool`、 `domain\user`、 `password`、および`domain\group`の値。
   ...
   * **ApplicationPool**: アプリケーション プールがサービスで使用します。 存在する場合、新しいものは作成されません。 既定値は**DefaultAppPool**します。
...
3. Power BI desktop が開き、URL が要求されます。 入力、 `http://localhost/<yourWebSite>` OData フィード用に作成した URL。 たとえば、入力`http://localhost/BizTalkOperationalDataService`します。 URL は、次のような検索します。  
