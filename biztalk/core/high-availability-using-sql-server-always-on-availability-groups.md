...
* SQL Server 2016 Enterprise、または SQL Server 2016 Standard (を参照してください**既知の制限**」を参照)
...
* MSDTC に対処するには、2 台のサーバーをホストしている 4 つの SQL インスタンスごとの最小値を使用して、BizTalk データベースの可用性グループの制限を構成できます。 使用することも[Azure ロード バランサーと複数の IP アドレス](https://docs.microsoft.com/azure/load-balancer/load-balancer-multivip-overview)です。 1 台のサーバー上のポート 1433 で 4 つの既定の SQL インスタンスを使用する場合は、4 つの IP アドレスが必要です。 1 つの IP アドレスに制限されている、同じサーバー上の複数の SQL インスタンスをホストする場合は、SQL インスタンスごとに、カスタム ポートを使用することを確認されます。 
...
* SQL Server 2016 Standard は、各 SQL AlwaysOn 可用性グループで 1 つのデータベースを 1 つだけをサポートします。 BizTalk では、多数のデータベースを使用するため SQL Server Enterprise edition は通常お勧めします。
* Azure Vm を使用するには場合、専用に使用する推奨が、MSDTC の TCP/IP ポートを固定します。 通常、以前のオペレーティング システムで使用して RPC ポートの範囲を制限していない固定 TCP/IP ポートを使用する場合ファイアウォールを簡略化し、ロード バランサーのルールと役に立ちます。 既知の低いポートとの競合を避けるためには、上位の固定ポートを使用してを検討してください (など > 20000)。 [1 つのポートのサポートの DTC を構成する](https://msdn.microsoft.com/library/windows/desktop/dd573191(v=vs.85).aspx)について説明します、`ServerTcpPort`レジストリ キー。 MSDTC の特定のポートに加え、メインの RPC ポート 135 も使用されます。 
