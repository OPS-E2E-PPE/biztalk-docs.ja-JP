...
1.  完全復旧モデルを使用するように SQL Server を構成して、[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] データベース バックアップ セットのデータの整合性を確保する必要があります。  詳細については、次を参照してください。[ログ配布](../core/log-shipping.md)です。  
...
3.  BizTalk 管理 (BizTalkMgmtDb) データベースに SQL Server Enterprise Manager または SQL Server Management Studio を使用すると変更、 **adm_OtherBackupDatabases**カスタム データベースの各の行を含むようにテーブルです。  
