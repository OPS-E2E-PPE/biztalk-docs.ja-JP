...
---
# <a name="admin-explorerom-biztalk-server-samples-folder"></a>Admin ExplorerOM (BizTalk Server Samples フォルダ)
Microsoft [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] のソフトウェア開発キット (SDK) の Admin\ExplorerOM フォルダーには、サンプルが含まれています。 このセクションでは、各 BizTalk エクスプローラー オブジェクト モデル管理サンプルが示す機能、サンプルをビルドおよび実行する方法、および予想される実行結果について詳しく説明します。  
  
> [!WARNING]
>  展開スクリプトは、展開後に不要になった場合は、削除する必要があります。 保持する必要のある管理スクリプトおよび他のスクリプトは、ACL によってセキュリティで保護し、厳重に監視する必要があります。  
  
> [!NOTE]
>  Microsoft.BizTalk.ExplorerOM.dll は、32 ビット プロセスと 64 ビット プロセスの両方をサポートします。  
...
-   [OrchestrationBinding (BizTalk Server サンプル)](../core/orchestrationbinding-biztalk-server-sample.md)です。 構成して、オーケストレーションの管理を示します。  
  
-   [PartnerManagement (BizTalk Server サンプル)](../core/partnermanagement-biztalk-server-sample.md)です。 作成のパーティを削除する方法について説明します。 ロールにパーティを参加および参加解除についても示します。  
