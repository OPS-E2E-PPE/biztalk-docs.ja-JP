---
title: "シングル サインオン: イベント 10847 |Microsoft ドキュメント"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: d6eb3fc8-a067-450e-875c-cc3bb3ecdd4a
caps.latest.revision: "7"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: a0bcf29217237729e115784ce85cd957ce280559
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 09/20/2017
---
# <a name="single-sign-on-event-10847"></a>シングル サインオン: イベント 10847
## <a name="details"></a>詳細  
  
|||  
|-|-|  
|製品名|エンタープライズ シングル サインオン|  
|製品バージョン|[!INCLUDE[btsSSOVersion](../includes/btsssoversion-md.md)]|  
|イベント ID|10847|  
|イベント ソース|ENTSSO|  
|コンポーネント|なし|  
|シンボル名|ENTSSO_E_MAPPINGS_ONLY_FOR_DOMAIN_USER_ACCOUNTS|  
|メッセージ テキスト|単独アプリケーションの場合、マッピングは Windows ドメイン ユーザーに対してのみ作成できます (ローカル アカウントやグループ アカウントは含まれません)。|  
  
## <a name="explanation"></a>説明  
 単独アプリケーションの場合、マッピングは Windows ドメイン ユーザーに対してのみ作成できます (ローカル アカウントやグループ アカウントは含まれません)。  
  
## <a name="user-action"></a>ユーザーの操作  
 個々 のアプリケーションの詳細については、次を参照してください。 [SSO 関連アプリケーション](../core/sso-affiliate-applications.md)です。