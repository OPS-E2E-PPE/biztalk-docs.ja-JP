---
title: オーケストレーションの作成 |Microsoft ドキュメント
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
helpviewer_keywords:
- orchestrations, creating
- creating, orchestrations
ms.assetid: 992418df-0acb-448f-9e12-ad857bf93f62
caps.latest.revision: 12
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 40af6f8c3d2dede604001dc422b58cd11256c68e
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 09/20/2017
ms.locfileid: "22238210"
---
# <a name="creating-orchestrations"></a>オーケストレーションの作成
このセクションでは、オーケストレーションの作成時やビジネス プロセス フローの設計時に実行する必要がある、さまざまなタスクについて説明します。  
  
## <a name="in-this-section"></a>このセクションの内容  
  
-   [オーケストレーション デザイナーでの作業](../core/working-in-orchestration-designer.md)  
  
-   [オーケストレーション フローのデザイン](../core/designing-orchestration-flow.md)  
  
-   [オーケストレーションで式の使用](../core/using-expressions-in-orchestrations.md)  
  
-   [オーケストレーションでメッセージの使用](../core/using-messages-in-orchestrations.md)  
  
-   [オーケストレーションで変数の使用](../core/using-variables-in-orchestrations.md)  
  
-   [オーケストレーションでの相関関係の使用](../core/using-correlations-in-orchestrations.md)  
  
-   [オーケストレーションでロール リンクの使用](../core/using-role-links-in-orchestrations.md)  
  
-   [オーケストレーションでポートを使用します。](../core/using-ports-in-orchestrations.md)  
  
-   [オーケストレーションでビジネス ルールを使用する方法](../core/how-to-use-business-rules-in-orchestrations.md)  
  
-   [オーケストレーションでマップを使用する方法](../core/how-to-use-maps-in-orchestrations.md)  
  
-   [オーケストレーションで Web サービスを使用する方法](../core/how-to-use-web-services-in-orchestrations.md)  
  
-   [トランザクションを使用して、例外の処理](../core/using-transactions-and-handling-exceptions.md)  
  
-   [BPEL4WS にエクスポートする方法](../core/how-to-export-bpel4ws.md)  
  
-   [BPEL4WS をインポートする方法](../core/how-to-import-bpel4ws.md)  
  
## <a name="see-also"></a>参照  
 [オーケストレーション デザイナーを使用してオーケストレーションの作成](../core/creating-orchestrations-using-orchestration-designer.md)