...
マップの特定の表示オプションの既定の設定を変更できます。 これらの設定は、Visual Studio を開くたびに使用できます。 ここでは、このようなオプションについて説明します。  
 ...
 色とフォントの既定の設定も設定できます。 詳細設定を選択する方法については、次を参照してください。[ハウツー色のカスタマイズおよび BizTalk マッパーでフォント](../core/how-to-customize-colors-and-font-in-biztalk-mapper.md)です。  
...
2.  **オプション**ダイアログ ボックスで、左ウィンドウで、展開、 **BizTalk マッパー**ノードをクリックして**全般**です。  
  
3.  チェック ボックスをクリックして、以下の表示オプションを有効または無効にします。  
    ...
    -   リレーションシップを別のグリッド ページに移動するときの確認メッセージ  
  
    -   先頭/末尾からの検索トラバーサルの確認メッセージ  
  
    -   スクリプト Functoid の指定スクリプト  
