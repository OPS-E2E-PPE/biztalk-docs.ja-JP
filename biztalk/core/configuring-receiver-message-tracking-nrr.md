...
### <a name="to-configure-message-tracking-for-inbound-as2-messages-and-outbound-mdn"></a>受信 AS2 メッセージと送信 MDN のメッセージ追跡を構成するには  
...
3.  選択**受信のエンコードされた AS2 メッセージの有効な NRR**にエンコードされた送信の AS2 メッセージを否認不可データベースに格納します。  
  
4.  選択**受信のデコードされた AS2 メッセージの有効な NRR**送信のデコードされた AS2 メッセージを否認不可データベースに格納します。  
