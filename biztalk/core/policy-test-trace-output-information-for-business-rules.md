...
### <a name="rule-engine-instance-identifier"></a>ルール エンジン インスタンス識別子  
 一意の識別子、 **RuleEngine**ルールの実行の実行環境を提供するインスタンス。  
...
> [!NOTE]
>  型と一致しません、ポリシーで使用される型のいずれかのファクトをアサートすると、アサート操作が「アサート-認識されないファクト」表示されます。  
...
-   **競合解決条件。** アクションが実行される相対順序を決定するルールの優先順位 (優先順位の高いアクションが最初に実行されます)。  
