---
title: "BizTalk Server 例外 Handling1 を使用して |Microsoft ドキュメント"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- exception handling
- BizTalk Server exception handling
- exceptions, handling
ms.assetid: d16ad8c2-f85a-4068-91ba-67623692df26
caps.latest.revision: "7"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 375f37023bae0dfdf35117a6233b93dba596a326
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 09/20/2017
---
# <a name="using-biztalk-server-exception-handling"></a>BizTalk Server 例外処理の使用
このセクションでは、BizTalk Server の例外処理オブジェクトを使用してバックエンド サーバーの例外をトラップする方法を説明します。  
  
## <a name="in-this-section"></a>このセクションの内容  
  
-   [基本的なオーケストレーション デザイン](../core/basic-orchestration-design1.md)  
  
-   [例外のメッセージを追加する方法](../core/how-to-add-a-message-for-an-exception2.md)  
  
-   [スコープ図形を追加する方法](../core/how-to-add-a-scope-shape3.md)  
  
-   [例外のキャッチ ブロックを追加する方法](../core/how-to-add-a-catch-exception-block4.md)  
  
-   [例外完了メッセージ](../core/completing-the-exception-message2.md)