...
## <a name="setting-up-the-permissions-for-a-wcf-receive-location-with-the-wcf-netmsmq-adapter"></a>Wcf-netmsmq アダプターを使用した場所を受信する WCF のアクセス許可を設定  
 ...
 サービスのキュー マネージャーは、それが所有する対象キューに宛先指定されたメッセージを受け入れて、格納します。 次に、サービスがターゲット キューからの読み取り要求を行い、キュー マネージャーがメッセージをサービスに配布します。 そのため、受信場所をホストする BizTalk ホスト インスタンスのサービス アカウントには、ターゲット キューから読み取るためのアクセス許可を与える必要があります。  
