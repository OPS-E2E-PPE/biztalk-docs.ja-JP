---
title: ビジネス アクティビティを使用して監視 |Microsoft ドキュメント
description: ビジネス アクティビティ監視 (BAM) BizTalk Server で作業を開始します。
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
ms.assetid: 8e01f54f-1d7f-4aed-ad14-2aeae716fd58
caps.latest.revision: 17
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 57ee67fec8e9379ce8fc62f5ec8c1d2d097924a0
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 09/20/2017
ms.locfileid: "22287002"
---
# <a name="using-business-activity-monitoring"></a>ビジネス アクティビティ監視の使用
ビジネス アクティビティ監視 (BAM) では、IT 環境や異種 IT 実装には関係なく、ビジネス プロセスを表示できます。 この点が、メッセージとインスタンスの監視などの運用の監視とは異なります。 BAM の重要な点は、できる限りリアルタイムに近い状態 (待機時間ゼロともいいます) でビジネス データを表示できることです。  
  
 開始する前にする必要がありますの情報と[ビジネス アクティビティ監視](../core/business-activity-monitoring.md)です。  
  
## <a name="next-steps"></a>次の手順
  
-   [BAM とは何ですか。](../core/what-is-bam.md)  
  
-   [インストールして BAM を構成します。](../core/installing-and-configuring-bam.md)  
  
-   [Implementg BAM ソリューション](../core/implementing-bam-solutions.md)  
  
-   [BAM 動的インフラストラクチャ](../core/bam-dynamic-infrastructure.md)  
  
-   [BAM ポータル](../core/bam-portal.md)  
  
-   [BAM を管理します。](../core/managing-bam.md)  
  
-   [BAM によるビジネス アクティビティの監視](../core/monitoring-business-activities-with-bam.md)  

-   [BAM コマンド ライン ツール](bam-command-line-tools.md)
  
-   [BAM をトラブルシューティングします。](../core/troubleshooting-bam.md)  
  
## <a name="see-also"></a>参照  
 [チュートリアル 4: ビジネス アクティビティ監視](http://msdn.microsoft.com/library/81d5e768-f8a6-4eb0-8e6c-64db47455476)