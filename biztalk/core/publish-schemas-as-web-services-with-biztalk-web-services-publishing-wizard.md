...
スキーマを Web サービスとして公開するのにには、BizTalk Web サービス公開ウィザードを使用します。  
...
4. **Web サービス**ページで、公開する Web サービスを定義します。 ツリーを使用して、 **Web サービスの説明**を追加、削除、名前の変更、および Web サービス説明ノードの編集 ダイアログ ボックス。 **情報** ダイアログ ボックスが選択したノードに関する情報し、現在のノードまたはサブノードでエラーが表示されます。  
   ...
   -   (Web サービスの説明) ツリーのルート ノードには、Web サービスのプロジェクト名がについて説明します。 仮想ディレクトリ名では、ルート ノードが既定の名前として使用されます。 Web サービスの説明を選択して変更できます**web サービスの説明の名前を変更**します。  
  
   -   新しい Web サービスを追加するには、右クリックし、 **Web サービスの説明**ノードをクリック**web サービスの追加**します。 これは、Web メソッドを指定せずに新しい Web サービスを作成します。 Web サービスの名前を変更するには、Web サービスのノードを右クリックして**web サービスの名前を変更**、し、enter キーを押して新しい名前を受け入れます。  
   ...
   -   名前を変更することができます、**要求**と**応答**ノードは、生成されたコードには影響しません。 スキーマを定義した後は名前を変更できます部分の要素、Web メソッド パラメータの名前を変更します。 生成された Web サービスのコードを表示して変更を確認できます。  
...
6. **Web サービスのプロパティ**ページで、 **web サービスのターゲット名前空間** ダイアログ ボックスでは、Web サービスのターゲットの名前空間を入力し、ウィザードがどのように指定する適切なボックスを選択しますSOAP ヘッダーと Web サービスのシングル サインオン サポートを処理します。 Web サービスの実装をさらにカスタマイズする場合は、クリックして**詳細**ボタンをクリックします。 次の詳細設定オプションが表示されます。  
   ...
   > [!NOTE]
   >  すべての Web サービスと Web メソッドの作成ウィザードのこのインスタンスを実行している場合にグローバルに適用されますが、SOAP ヘッダー オプションのいずれか選択します。  
