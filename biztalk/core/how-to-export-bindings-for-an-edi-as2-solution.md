...
2. 構成のコピー元 をポイントする BizTalk アプリケーションを右クリックして**エクスポート**、 をクリックし、**バインド**します。  
...
4. すべてのパーティをエクスポートして、その機密性が高くないプロパティには、オーケストレーションがバインドされていない場合でもクリックして場合**グローバル パーティ情報**します。  
  
   > [!NOTE]
   >  クリックしない場合**グローバル パーティ情報**、[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]バインド ファイルに、オーケストレーションにバインドされているパーティのプロパティがエクスポートされます。 ただし、これはエクスポートされません、オーケストレーションにバインドされていないパーティをクリックしない**グローバル パーティ情報**します。  
   > 
   > [!NOTE]
   >  バインド ファイルを生成中に、 **グローバル パーティ情報**プロパティが選択されているコンピューターで定義されたすべてのパーティの構成が含まれます。 コンピュータで定義されたパーティの完全なセットのサブセットの構成は、エクスポートできません。  
  
5. クリックして**OK**バインドをエクスポートします。  
...
## <a name="edi-and-as2-nodes-in-the-biztalk-server-binding-file"></a>BizTalk Server バインド ファイルの EDI および AS2 ノード  
 構成をエクスポートする場合、 **グローバル パーティ情報**プロパティを選択すると、[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]次のノードを含むバインド ファイルが生成されます。  
