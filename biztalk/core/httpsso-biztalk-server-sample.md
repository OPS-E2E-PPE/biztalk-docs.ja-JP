...
1. クリックすると**完了**をこのサンプルのユーザー インターフェイスを構成するウィザード アプリケーションで Internet Explorer のインスタンスが起動し、BizTalk HTTP Receive DLL の URL を渡します。  
...
|SsoSample.cs|このサンプルの多くを構成する HTTP SSO グラフィカル アプリケーションの最上位の Microsoft Visual C# ソース ファイルです。 このファイルは、という名前のクラスで、SSO 構成コードを含む**SsoConfigurator**が最終的にこのサンプルのポイント。|  
...
2. Microsoft SQL Server の Enterprise Manager を使用して開き、**従業員**Northwind サンプル データベースのテーブルが表示され、作成したサンプル データをさまざまな含むローカルの Windows アカウントに対応する行を追加列です。 Employees テーブルの LastName 列に追加した値は、手順 1. で追加したローカル Windows アカウントのユーザー名と同じである必要があります。  
...
5. **[ビルド]** メニューの **[ソリューションのビルド]** をクリックします。  
   ...
   - **Microsoft.BizTalk.ExplorerOM**します。 既定では、Microsoft.BizTalk.ExplorerOM.dll ファイルは、フォルダーにあります[!INCLUDE[btsBiztalkServerPath](../includes/btsbiztalkserverpath-md.md)]Developer Tools\\します。  

   - **Microsoft.BizTalk.SSOClient.Interop**します。 既定では、Microsoft.BizTalk.Interop.SSOClient.dll ファイルは、フォルダーにあります\< *%programfiles%*\>でシングル サインオン \Common Files\Enterprise\\します。  
...
2. クリックして [ようこそ] ページで、IIS、SSO、および BizTalk を構成するため、既定の設定を受け入れる**次**します。  

3. IIS の構成 ページで、作成、およびクリックする 2 つの IIS 仮想ディレクトリの既定の設定を受け入れる**次**します。  

4. SSO 構成 ページを使用してアクセスできる関連アプリケーションの既定の設定をそのまま使用、**アプリケーションを追加**ボタンをクリックします。  

5. SSO 構成 ページで、ほとんどのユーザーのマッピングの既定の設定をそのまま使用を使用してアクセス、**追加マッピング**ボタンをクリックします。 このサンプルをビルドおよび初期化したときに追加したローカル Windows アカウントに基づいて、次の 2 つの設定に値を指定します。  
...
6. [SSO 構成] ページで、次のようにクリックします。**次**します。  

7. BizTalk の構成 ページで、送信の既定の設定をそのまま使用し、受信ポート、およびをクリックして**次**します。  

   > [!NOTE]
   >  既定の送信ポート設定を変更する**EmployeeData.aspx**に**ValidateUser.aspx**の Employee テーブルから取得されたサンプル データではなく、シンプルなユーザー検証を表示する場合、Northwinds SQL データベース。 この変更の変更内容をクリックした後、ブラウザーに表示される出力の性質**完了**手順 9. でします。  

8. 実行される IIS、SSO、および BizTalk の各構成に対応する状態メッセージを確認します。 このフェーズで実行されるコードを検索することができます、 **IisConfigurator**、 **SsoConfigurator**、および**BtsConfigurator**ファイル SsoSample.cs で定義されているクラス。 構成が完了すると、クリックして**次**します。  

9. ウィザード アプリケーションの最後のページの既定の設定をそのまま使用**でブラウザーを開始**(チェック ボックスがオンと、テキスト ボックスに URL http://localhost/SsoSampleBizTalkHttpReceive/BTSHttpReceive.dll? <message/>)、順にクリックします**完了**します。  
...
4.  Ssomanage アプリケーションを使用して (\Program Files\Common \enterprise シングル サイン オンである\\) このサンプルの SSO アプリケーションを削除します。  
