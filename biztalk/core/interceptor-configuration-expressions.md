...
BAM インターセプター構成ファイルでは、フィルター式を使用して、アクティビティを識別し、データ式を使用して、ストレージのデータ要素を構築、関連付け ID または継続トークン、または同様の目的として使用します。 その目的に関係なく、個々の式は `expression` 要素によってインターセプタ構成ファイル内に指定され、式には後置表記法とも呼ばれる逆ポーランド表記法 (RPN: Reverse Polish Notation) を使用した 1 つまたは複数の演算子が含まれています。  
...
## <a name="reverse-polish-notation-in-the-interceptor-configuration-file"></a>インターセプタ構成ファイル内での逆ポーランド表記法  
 2 つの式で作成、インターセプター構成ファイル: 式と式のデータをフィルター処理します。 フィルタ式は RPN 式の結果を処理してブール値 `true` または `false` を返すのに対し、データ式はスタック上の単一の値を処理します。  
  
### <a name="filter-expressions"></a>フィルタ式  
 フィルタ式はブール値 `true` または `false` を評価し、特定のイベントを識別して WF アプリケーションまたは WFC アプリケーションを追跡するために使用されます。 WF アプリケーションでは、一般にアクティビティ名とイベントに基づいてフィルタ処理します。 たとえばを選択することがあります、 **FoodAndDrinksPolicy**アクティビティが**Closed**します。 WF 操作を使用すると、フィルタを次のように表現できます。  
...
```  
 ...
 仮定すると次のようにこの式を評価が最後に、 **GetActivityName** "dessertpolicy"と**GetActivityEvent** "Closed"が返されます。  
...
### <a name="data-expressions"></a>データ式  
 ...
 これは、BAM アクティビティ データベースをラベル付きタイム スタンプで更新するための一般的な要件です。 形式の文字列でイベントの開始時刻をキャプチャするたとえば、"開始: \<EventTime\>"。 これを行うには、次のような式を使用する必要があります (+ は連結を表します)。  
...
|連結|連結|"開始: 2006-09-27T12:00:34.000Z"|  
 ...
 更新コマンドによって使用される値になります"開始: 2006-09-27T12:00:34.000Z"。  
