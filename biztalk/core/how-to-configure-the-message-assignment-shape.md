...
---
# <a name="how-to-configure-the-message-assignment-shape"></a>メッセージの割り当て図形を構成する方法
![](../core/media/ebiz-orch-assign.gif "ebiz_orch_assign")  
メッセージの割り当て図形  
  
> [!NOTE]
>  A**メッセージの割り当て**図形を配置できる内でのみ、**メッセージの構築**図形です。 デザイン サーフェイスでは、メッセージの割り当てをその他の場所にドラッグした場合、新しい**メッセージの構築**図形が作成されます。  
  
### <a name="to-configure-a-message-assignment-shape"></a>メッセージの割り当て図形を構成するには  
  
1.  BizTalk 式エディターが表示されていない場合を右クリックし、**メッセージの割り当て**図形をクリックして**式の編集**か、[プロパティ] ウィンドウをクリックして、**省略記号**(**...**) ボタンをクリックして、**式**プロパティです。  
