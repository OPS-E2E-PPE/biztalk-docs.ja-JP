---
redirect_url: /biztalk/core/installing-biztalk-adapter-for-tibco-enterprise-message-service
redirect_document_id: True
ROBOTS: NOINDEX
ms.openlocfilehash: 8d0e192e4a17427ec672bf334e75205384dd5eae
ms.sourcegitcommit: 6b6d905bbef7796c850178e99ac293578bb58317
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 10/17/2017
---
# <a name="preparing-tibco-enterprise-message-service"></a>TIBCO Enterprise Message Service を準備する
TIBCO Enterprise Message Service システムを使用する Microsoft BizTalk Adapter for TIBCO Enterprise Message Service を準備する方法については、次を参照してください[Microsoft BizTalk Adapters for Enterprise Applications インストール ガイド](../adapters-and-accelerators/install-configure-biztalk-adapters-enterprise-applications.md)。.  
  
## <a name="see-also"></a>参照  
 [BizTalk Adapter for TIBCO Enterprise Message Service のインストール](../core/installing-biztalk-adapter-for-tibco-enterprise-message-service.md)   
 [計画とアーキテクチャ](../core/planning-and-architecture16.md)   
 [作業の開始](../core/getting-started-with-biztalk-adapter-for-tibco-enterprise-message-service.md)