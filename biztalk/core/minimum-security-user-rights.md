...
##  <a name="BKMK_GroupRole"></a>グループとロールのメンバーシップ  
 次の表では、BizTalk Server でタスクを実行する必要があります。 最小セキュリティ ユーザー権限について説明します。  
...
|ホストの "ホストの追跡" プロパティの変更|BizTalk Server 管理者<br />BAM プライマリ インポート データベース、BizTalk メッセージ ボックス データベース、および BizTalk 追跡データベースの db_securityadmin SQL Server データベース ロール|  
...
|サーバーの追加または削除|BizTalk Server 管理者<br />のコンピューター ローカルの管理者を追加または削除します。|  
...
|データベースのバックアップ|のデータベース BTS_BACKUP_USERS ロール<br />BizTalk 管理データベースをホストする SQL Server の sysadmin SQL Server ロール。 **注:**を SQL Server の各インスタンスにマップされたユーザーにドメイン アカウントまたはローカル アカウントで実行する SQL Server エージェント サービスを構成する必要があります。|  
...
|使用して**メッセージの検索**クエリ|BizTalk Server 管理者|  
|使用して**ビルドのクエリ**|BizTalk Server 管理者|  
...
|BM.exe の実行 (Analysis Services データベースがある場合)|BAM プライマリ インポート、BAM スター スキーマ、および BAM アーカイブ データベースの db_owner SQL Server データベース ロール<br />BAM Analysis Services データベースでの OLAP 管理者|  
|BAM ビュー用アカウントの作成|BAM プライマリ インポート データベースの db_owner SQL Server データベース ロール<br />BAM Analysis Services データベースでの OLAP 管理者|  
|**ルール エンジン (発行規則)**||  
|ポリシーの展開および展開解除、セキュリティ関連アイテムの操作|のルール エンジンのデータベース RE_ADMIN_USERS SQL Server データベース ロール|  
...
|2|のレベル 1 に付与されたユーザー権限<br />-すべての SQL Server の securityadmin SQL Server ロール<br />-db_securityadmin ロールおよび db_accessadmin SQL Server データベース ロール、BizTalk 追跡、ルール エンジン、BizTalk 管理、BAM プライマリ インポート データベースと BizTalk メッセージ ボックス データベースで<br />-すべての BizTalk メッセージ ボックス データベース上の db_ddladmin SQL Server データベース ロール<br />-SSO 関連管理者|-作成し、BizTalk ホストの削除<br />ホスト追跡プロパティの変更<br />追加し、サーバーの削除<br />追加ハンドラーと受信ハンドラーの削除<br />-アダプターを追加します。|  
|3|のレベル 2 に付与されたユーザー権限<br />のすべての BizTalk Server ランタイム コンピューター上ローカル管理者|-作成し、ホスト インスタンスを削除|  
