...
|**/入力**(または **/T**、「解説」を参照してください)|可|**System.BizTalk:BizTalkBinding** (この値小文字は区別されません)。|  
|**/ソース**(または **/So**、「解説」を参照してください)|可|バインド ファイルの完全パス (ファイル名を含む)。 パスには、スペースが含まれている場合は、二重引用符 (") で囲む必要があります。|  
