...
|BizTalk Server: BizTalk の追跡データのサイズ|BizTalk Server では、システムの詳細と詳細データを処理するサイズを大きく、BizTalk 追跡データベース (BizTalkDTADb) は続行されます。 サイズが増加してもチェックされないので、システム パフォーマンスが低下し、TDDS (Tracking Data Delivery Service) でエラーが発生することがあります。 一般的な追跡データに加えて、追跡メッセージもメッセージ ボックス データベースに累積されます。これにより、ディスクのパフォーマンスが低下します。 この分析は 5 MB 以上の追跡に 1 時間あたりの上昇傾向のチェックのデータのサイズ。<br /><br /> リファレンス:<br /><br /> アーカイブおよび BizTalk 追跡でデータベースを削除[http://go.microsoft.com/fwlink/?LinkID=153816](http://go.microsoft.com/fwlink/?LinkID=153816)|  
