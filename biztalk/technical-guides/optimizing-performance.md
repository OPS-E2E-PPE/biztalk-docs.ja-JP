---
title: "パフォーマンスの最適化 |Microsoft ドキュメント"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: bafa119b-187e-4595-a673-358dc0a109b7
caps.latest.revision: "11"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 4e79c318d7cd12d799459535914046da98819561
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 09/20/2017
---
# <a name="optimizing-performance"></a>パフォーマンスの最適化
Windows オペレーティング システム、SQL Server の既定のインストール[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]、実稼働の最適なパフォーマンスを提供する、IIS を大幅に最適化できますと[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]環境。 WCF 構成パラメーターは、パフォーマンスが大幅に向上を提供する既定の設定からチューニングできます。 このセクションでは特定のパフォーマンスの最適化、実稼働環境を展開するときに従う必要がありますを[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]ソリューションです。  
  
## <a name="in-this-section"></a>このセクションの内容  
  
-   [オペレーティング システムのパフォーマンスを最適化します。](../technical-guides/optimizing-operating-system-performance.md)  
  
-   [ネットワーク パフォーマンスを最適化します。](../technical-guides/optimizing-network-performance.md)  
  
-   [IIS のパフォーマンスを最適化します。](../technical-guides/optimizing-iis-performance.md)  
  
-   [データベースのパフォーマンスを最適化します。](../technical-guides/optimizing-database-performance.md)  
  
-   [BizTalk Server のパフォーマンスを最適化します。](../technical-guides/optimizing-biztalk-server-performance.md)  
  
-   [オーケストレーションのパフォーマンスを最適化します。](../technical-guides/optimizing-orchestration-performance.md)  
  
-   [WCF Web サービスのパフォーマンスを最適化します。](../technical-guides/optimizing-wcf-web-service-performance.md)  
  
-   [BizTalk Server アプリケーションの最適化](../technical-guides/optimizing-biztalk-server-applications.md)  
  
-   [Windows PowerShell スクリプト](../technical-guides/windows-powershell-scripts.md)