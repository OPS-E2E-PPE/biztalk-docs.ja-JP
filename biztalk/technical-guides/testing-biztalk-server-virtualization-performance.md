...
- **テストのシナリオ 3:別々 の物理的な HYPER-V ホスト上の BizTalk サーバー/仮想 SQL Server が仮想**-仮想化環境で BizTalk Server と SQL Server の両方を実行中のパフォーマンスに与える影響を判断する 3 番目のシナリオを行いました。 テストは、HYPER-V 仮想マシンで実行されている SQL Server インスタンスでホストされる BizTalk データベースで、HYPER-V 仮想マシンで実行されている BizTalk Server を使用して実行されました。 このシナリオでは、BizTalk Server の仮想マシンと SQL Server 仮想マシンに別々 の物理 HYPER-V ホストでホストされます。  
